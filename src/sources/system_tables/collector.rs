...
pub struct Collector {
    ...
    out: SourceSender,
    shared_pool: sqlx::mysql::MySqlPool,
}
...
impl Collector {
    ...
    pub async fn run(self) {
        let enabled_tables: Vec<&str> = self
            .tables
            .iter()
            .filter(|t| t.enabled)
    ...
    async fn collect_table_data(
        ...
        info!(
            "Starting collection for table {} with {}s interval from instance {}",
            table.source_table, interval, instance
        );

        // Get table schema once at startup
        let column_types = match Self::get_table_schema(&table, &pool).await {
            Ok(schema) => {
                debug!(
                    "Retrieved schema for table {} from instance {}: {} columns",
                    table.source_table,
                    instance,
                    schema.len()
                );
            ...
            Err(e) => {
                error!(
                    "Failed to get schema for table {} from instance {}: {}",
                    table.source_table, instance, e
                );
                return;
            }
        };
        ...
        loop {
            ...
            match Self::query_table_data(&table, &pool, &column_types, &instance).await {
                Ok(data) => {
                    if !data.is_empty() {
                        ...
                        debug!(
                            "Collected {} rows from table {}, buffer size: {}",
                            data_len,
                            table.source_table,
                            event_buffer.len()
                        );
                        ...
                        if should_send && !event_buffer.is_empty() {
                            ...
                            if let Err(e) = out.send_batch(events_to_send).await {
                                error!(
                                    "Failed to send batch for table {} from instance {}: {}",
                                    table.source_table, instance, e
                                );
                            } else {
                                info!(
                                    "Sent batch of {} events for table {} from instance {}",
                                    batch_size_sent, table.source_table, instance
                                );
    ...
    async fn query_table_data(
        ...
        debug!(
            "Query returned {} rows for {}.{} from instance {}",
            rows.len(),
            table.source_schema,
            table.source_table,
            instance
    ...
    fn create_event(
        ...
        if include_schema {
            ...
            info!(
                "Included schema metadata for table {} ({} fields)",
                table.dest_table, schema_field_count
            );
        }
