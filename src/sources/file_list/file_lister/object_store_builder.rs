...
/// Build ObjectStore based on endpoint and cloud provider
...
fn build_s3_store(url: &Url) -> vector::Result<Arc<dyn ObjectStore>> {
    ...
    if let Some(region) = std::env::var("AWS_REGION").ok() {
...
fn build_oss_store(url: &Url) -> vector::Result<Arc<dyn ObjectStore>> {
    ...
    let endpoint = std::env::var("OSS_ENDPOINT")
        .ok()
        .ok_or_else(|| "OSS_ENDPOINT environment variable is required for Aliyun OSS".to_string())?;
