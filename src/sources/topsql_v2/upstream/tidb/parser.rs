...
impl TopSqlSubResponseParser {
    ...
    fn parse_top_ru_record(
        ...
        let keyspace_name_str = Self::extract_top_ru_keyspace(&record);
    ...
    fn extract_top_ru_keyspace(
        ...
        record
            .user
            .split_once('@')
            .map(|(user_with_keyspace, _)| user_with_keyspace)
            .unwrap_or(record.user.as_str())
            .split_once('.')
...
mod tests {
    use super::*;
    use crate::sources::topsql_v2::upstream::tidb::proto::{
        TopRuRecord, TopRuRecordItem, TopSqlRecordItem,
    };
    ...
    #[test]
    fn test_parse_top_ru_record_falls_back_to_user_prefix_for_keyspace() {
        let ru_record = TopRuRecord {
            keyspace_name: vec![],
            user: "test_keyspace.root@%".to_string(),
            sql_digest: b"sql_digest_123".to_vec(),
            plan_digest: b"plan_digest_456".to_vec(),
            items: vec![TopRuRecordItem {
                timestamp_sec: 1709646900,
                total_ru: 54.4,
                exec_count: 5,
                exec_duration: 123456789,
            }],
        };
        ...
        assert_eq!(
            log.get(LABEL_USER),
            Some(&LogValue::from("test_keyspace.root@%"))
        );
        assert_eq!(log.get(METRIC_NAME_TOTAL_RU), Some(&LogValue::from(54.4)));
    }
}
