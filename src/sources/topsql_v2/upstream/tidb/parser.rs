...
fn decode_keyspace_name(keyspace_name: &[u8]) -> Option<String> {
    ...
    String::from_utf8(keyspace_name.to_vec()).ok()
}
...
impl UpstreamEventParser for TopSqlSubResponseParser {
    ...
    fn parse(
        ...
        match response.resp_oneof {
            ...
            Some(RespOneof::TopRuRecords(top_ru_records)) => {
                Self::parse_top_ru_records(top_ru_records)
            }
            None => vec![],
        }
    }
...
impl TopSqlSubResponseParser {
    fn parse_tidb_record(record: TopSqlRecord, instance: String) -> Vec<LogEvent> {
        let keyspace_name_str = decode_keyspace_name(&record.keyspace_name);
        let mut events = vec![];
        ...
        for item in &record.items {
            ...
            if let Some(keyspace_name) = &keyspace_name_str {
                log.insert(LABEL_KEYSPACE, keyspace_name.clone());
            }
            log.insert(
                LABEL_SQL_DIGEST,
    ...
    fn parse_tidb_sql_meta(sql_meta: SqlMeta) -> Vec<LogEvent> {
        ...
        let keyspace_name = decode_keyspace_name(&sql_meta.keyspace_name);
        let mut event = Event::Log(LogEvent::default());
        let log = event.as_mut_log();
    ...
    fn parse_tidb_plan_meta(plan_meta: PlanMeta) -> Vec<LogEvent> {
        ...
        let keyspace_name = decode_keyspace_name(&plan_meta.keyspace_name);
    ...
    fn parse_top_ru_records(
        top_ru_records: crate::sources::topsql_v2::upstream::tidb::proto::ReportTopRuRecords,
    ) -> Vec<LogEvent> {
        let mut events = vec![];
        let mut date = String::new();
...
mod tests {
    use super::*;
    use crate::sources::topsql_v2::upstream::tidb::proto::{
        PlanMeta, ReportTopRuRecords, SqlMeta, TopRuRecord, TopRuRecordItem, TopSqlRecordItem,
    };
    ...
    #[test]
    fn test_parse_tidb_sql_meta_preserves_keyspace() {
    ...
    #[test]
    fn test_parse_tidb_plan_meta_preserves_keyspace() {
        let plan_meta = PlanMeta {
            plan_digest: b"plan_digest_456".to_vec(),
            normalized_plan: "Point_Get".to_string(),
            encoded_normalized_plan: "plan_binary".to_string(),
            keyspace_name: b"test_keyspace".to_vec(),
        };
