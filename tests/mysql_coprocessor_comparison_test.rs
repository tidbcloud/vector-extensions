...
async fn test_mysql_coprocessor_one_to_one_comparison() {
    ...
    let comparison_result = compare_data_by_digest(&normalized_mysql_data, &normalized_coprocessor_data);

    // 4. 输出对比结果
    println!("📊 对比结果:");
    println!("  总行数对比: CoprocessorCollector={}, MySQL={}", 
             coprocessor_data.len(), mysql_data.len());
    ...
    println!("  总字段差异数: {}", comparison_result.total_field_differences);
    ...
    for (row_index, row_result) in comparison_result.row_results.iter().enumerate() {
        ...
        if !row_result.field_differences.is_empty() {
            ...
            for diff in &row_result.field_differences {
                println!("      {}: CoprocessorCollector={:?}, MySQL={:?}", 
    ...
    output_csv_comparison_by_digest(&normalized_coprocessor_data, &normalized_mysql_data, &comparison_result);
...
// 标准化 timestamp 字段
fn normalize_timestamp_fields(data: &[HashMap<String, serde_json::Value>]) -> Vec<HashMap<String, serde_json::Value>> {
    let timestamp_fields = ["SUMMARY_BEGIN_TIME", "SUMMARY_END_TIME", "FIRST_SEEN", "LAST_SEEN"];
        ...
        for field_name in &timestamp_fields {
            if let Some(value) = normalized_row.get(*field_name) {
                if let Some(timestamp_str) = value.as_str() {
                    // 尝试解析不同的时间格式
                    if let Some(normalized_timestamp) = parse_timestamp_to_microseconds(timestamp_str) {
                        // 保存微秒时间戳用于对比
                        normalized_row.insert(field_name.to_string(), serde_json::Value::String(normalized_timestamp.clone()));
                        
                        // 额外保存本地时间字符串用于查看
                        let local_time_str = format!("{}_LOCAL", field_name);
                        if let Some(local_time) = microseconds_to_local_time_string(&normalized_timestamp) {
                            normalized_row.insert(local_time_str, serde_json::Value::String(local_time));
                        }
                    }
                }
            }
    ...
    match File::create(filename) {
        Ok(mut file) => {
            ...
            for digest in coprocessor_by_digest.keys() {
                ...
                let mysql_rows = mysql_by_digest.get(digest).map(|v| v.as_slice()).unwrap_or(&[]);
                ...
                for coprocessor_row in coprocessor_rows.iter() {
                    ...
                    for field_name in &sorted_fields {
                        let value = coprocessor_row.get(field_name.as_str())
                            .map(|v| v.to_string()).unwrap_or_else(|| "NULL".to_string());
                ...
                for mysql_row in mysql_rows.iter() {
                    ...
                    write!(file, ",{}", digest).unwrap();
                    for field_name in &sorted_fields {
                        let value = mysql_row.get(field_name.as_str())
                            .map(|v| v.to_string()).unwrap_or_else(|| "NULL".to_string());
                        let escaped = escape_csv_value(&value);
                        write!(file, ",{}", escaped).unwrap();
                    }
...
// 按 digest 分组数据
fn group_data_by_digest(data: &[HashMap<String, serde_json::Value>]) -> std::collections::HashMap<String, Vec<HashMap<String, serde_json::Value>>> {
    let mut grouped = std::collections::HashMap::new();
    
    for row in data {
        if let Some(digest) = row.get("DIGEST") {
            if let Some(digest_str) = digest.as_str() {
                grouped.entry(digest_str.to_string())
...
fn compare_data_one_to_one(
    ...
    // 逐行对比
    for (row_index, (mysql_row, coprocessor_row)) in mysql_data.iter().zip(coprocessor_data.iter()).enumerate() {
        ...
        for field_name in &all_fields {
            ...
            if mysql_value != coprocessor_value {
                ...
                // 记录字段差异统计
                *result.field_differences.entry(field_name.to_string()).or_insert(0) += 1;
            }
        }
    ...
    for row in rows {
        ...
        for (i, column) in row.columns().iter().enumerate() {
            ...
            let value = match row.try_get::<Option<chrono::DateTime<chrono::Utc>>, _>(i) {
                Ok(Some(dt)) => serde_json::Value::String(dt.format("%Y-%m-%d %H:%M:%S").to_string()),
...
fn compare_data_by_digest(
    ...
    let mut mysql_by_digest: HashMap<String, Vec<&HashMap<String, serde_json::Value>>> = HashMap::new();
    for row in mysql_data {
        if let Some(digest) = row.get("DIGEST").and_then(|v| v.as_str()) {
            mysql_by_digest.entry(digest.to_string()).or_insert_with(Vec::new).push(row);
        }
    }
    
    // 按 digest 分组 CoprocessorCollector 数据
    let mut coprocessor_by_digest: HashMap<String, Vec<&HashMap<String, serde_json::Value>>> = HashMap::new();
    for row in coprocessor_data {
        if let Some(digest) = row.get("DIGEST").and_then(|v| v.as_str()) {
            coprocessor_by_digest.entry(digest.to_string()).or_insert_with(Vec::new).push(row);
        }
    }
    
    println!("📊 CoprocessorCollector 数据按 digest 分组: {} 个不同的 digest", coprocessor_by_digest.len());
    println!("📊 MySQL 数据按 digest 分组: {} 个不同的 digest", mysql_by_digest.len());
    ...
    for (digest, mysql_rows) in &mysql_by_digest {
        ...
        if let Some(coprocessor_rows) = coprocessor_by_digest.get(digest) {
            println!("  CoprocessorCollector 行数: {}, MySQL 行数: {}", coprocessor_rows.len(), mysql_rows.len());
            ...
            for (i, mysql_row) in mysql_rows.iter().enumerate() {
                if i < coprocessor_rows.len() {
                    ...
                    for diff in &row_result.field_differences {
                        *field_differences.entry(diff.field_name.clone()).or_insert(0) += 1;
                    }
...
async fn collect_data_via_coprocessor(
    ...
    println!("🔍 通过 CoprocessorCollector 获取 {}.{} 的数据", schema, table);
    ...
    println!("📊 CoprocessorCollector 收集完成: {} 行数据，耗时 {}ms", 
             result.data.len(), 
